...
import com.jay.kanaiya.HubPvPPlus.itemguilib.CustomItemManager;
...
public class PvPManager {
    ...
    public CustomItem getItemFromConfig(String name) {
        ...
        CustomItemManager customItemManager = CustomItemManager.get();
        CustomItem item = customItemManager.createCustomItem(new ItemStack(Material.valueOf(material)));

        String itemName = instance.getConfig().getString("items." + name + ".name");
        if (itemName != null && !itemName.isEmpty()) {
            item.setName(StringUtil.colorize(itemName));
        }

        List<String> lore = instance.getConfig().getStringList("items." + name + ".lore");
        if (!(lore.size() == 1 && lore.get(0).isEmpty())) {
            item.addLore(StringUtil.colorize(lore));
        }
        ...
        if (enchants != null && !enchants.isEmpty()) {
            for (String enchant : enchants) {
                ...
                if (split.length < 2) {
                    instance.getLogger().warning("Invalid enchantment format for item " + name);
                    continue;
                }
    ...
    public void enablePvP(Player player) {
        ...
        if (getOldData(player) != null) {
            getOldPlayerDataList().remove(getOldData(player));
        }
    ...
    public void giveWeapon(Player p) {
        p.getInventory().setItem(HubPvPPlus.instance().getConfig().getInt("items.weapon.slot") - 1, getWeapon().getItemStack());
    }
